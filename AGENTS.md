...
- For lightweight standalone checks of 6x6 `黒先白死` candidates, use `go_tsumego_verifier.py` and `python3 -m unittest test_go_tsumego_verifier.py`. This verifier checks legal moves, liberties, captures, principal variations, target capture, simple alternate first moves, target history, limited two-eye status, and expected final rows; it is not a full life-and-death engine.
- Do not accept a new tsumego only because the verifier returns `ok: true`. Always inspect `warnings` / `warningDetails`, `searchCompleteness`, `firstMoveAnalysis`, `whiteDefenses`, and `targetEyeAnalysis`. Reject or revise when `PRUNING`, `SEARCH_CUTOFF`, `AMBIGUOUS_LIFE_DEATH`, or `UNSUPPORTED_SUCCESS_CONDITION` appears.
- For `黒先白死` problems, set the verifier `successCondition` explicitly. Use `capture-target` only for true capture problems; use `prevent-target-two-eyes`, `prevent-white-two-eyes`, or `capture-or-prevent-two-eyes` for two-eye prevention problems.
